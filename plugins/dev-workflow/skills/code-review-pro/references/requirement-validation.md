...
Detection order remains PR links, branch identifiers, then commit identifiers. If unresolved, inspect `.docs/connectors/ado/context.md` for a candidate. User-provided requirement text overrides inferred context.
...
2. Use `.docs/connectors/ado/context.md` (maintained by the `ado-context` skill) to map the resolved item's parent Feature/Epic to its design-doc file(s).
