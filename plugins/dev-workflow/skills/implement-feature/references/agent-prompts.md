...
## Requirement
{full requirement.md content — see context size guidelines}
...
4. Write to {artifact-folder}/plan.md. Every AC must map to at least one WP.
`
})
```
...
- **ACs**: {relevant ACs}
- ONLY modify listed files. Follow existing patterns. No unnecessary abstractions.
`
})
```
...
- ONLY modify listed files. Address every finding. No unrelated changes.
`
})
```
...
## Requirement
{full requirement.md — see context size guidelines}

## Acceptance Criteria
{all ACs from plan.md}
