...
Use this template when writing `{artifact-folder}/{feature-name}-requirement.md` at the end of Phase 1. Filename embeds the feature name (e.g., `user-analytics-requirement.md`) so it self-identifies. Legacy `requirement.md` is read on input but not written for new files.
