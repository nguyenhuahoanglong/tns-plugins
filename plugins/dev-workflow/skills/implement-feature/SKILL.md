...
Resolve the artifact folder **once** at the start, before any file writes:

| Input | Artifact folder |
|-------|----------------|
| **File path** | Parent directory of the file |
| **Folder path** | The folder itself |
| **Work item ID** or **inline text** | `.plans/{feature-name}/` in project root |

Create the folder if it doesn't exist. All artifacts go here: `plan.md`, `requirement.md`, `qa/`, `reviews/`.
...
- **Folder** → scan for existing artifacts:
  - If `requirement.md` exists → read it and use as starting input (treat as file path input)
  - If `plan.md` also exists → check its status to decide whether to resume or restart
...
- Write `{artifact-folder}/requirement.md` using `references/requirement-template.md`
  - **Skip** if the artifact folder already contains a `requirement.md` that was used as input — update in place instead
...
Spawn a **Plan agent** with `{artifact-folder}/requirement.md` content and codebase context. See `references/agent-prompts.md` for dispatch template and context size guidelines.

- Create `{artifact-folder}/plan.md` following `references/plan-template.md`
...
1. Spawn **code-implementer** agents in parallel — one per WP in the wave
   - Each receives: WP scope, file list, requirement context, coding standards
...
3. Update each WP status in `plan.md` to `complete`
...
1. Read `{artifact-folder}/requirement.md` + acceptance criteria
...
1. Set `{artifact-folder}/plan.md` status to `done`, check off all ACs
