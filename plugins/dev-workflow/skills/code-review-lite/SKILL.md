...
Accept optional `Escalation Policy: auto|ask`; omitted means `ask`. Record whether the value was
`omitted-default` or `user-authored`; only an explicitly user-authored `auto` may route to Pro.
...
4. Classify risk families from the production allowlist. For multiple families, apply the
   escalation decision table before Lite gates: explicitly user-authored `auto` invokes `code-review-pro`;
   `ask` explains every family and why Pro is recommended, then pauses. An accepted ask invokes Pro.
