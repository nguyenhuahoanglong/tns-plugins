...
- **Review boundary** — Changed-file list and diff content or review context path
...
6. Reverse-map every material changed behavior to one classification:
   - `Direct requirement` — implements or corrects an explicit direct criterion.
   - `Necessary collateral` — required to deliver a direct criterion safely, but is not a new criterion.
   - `Unrelated` — has no evidence-backed necessity for a direct criterion.
   - `Unclear` — available evidence cannot establish the relationship.
...
- **MEDIUM** — Material scope drift or consequential risk lacking complete exposure evidence.
...
- **Tests are evidence** — Missing tests mean `Unproven`, not an automatic defect, severity, or finding.
- **Collateral is not criteria** — `Necessary collateral` may justify a delta but must never be promoted into acceptance criteria.
- **Material-only** — Do not emit stylistic notes, exhaustive passing traces, or findings without a decision-relevant impact.
